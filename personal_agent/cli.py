"""CLI entry point for FlossWare coding-agent-ai.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        prog="pa",
        description="FlossWare provider-neutral AI coding agent",
    )
    ...
    parser.add_argument("--files", "-f", nargs="*", default=[], help="Specific files to focus on")
    parser.add_argument("--commands", "-c", nargs="*", default=[], help="Test commands to run after changes")
    parser.add_argument("--max-iter", "-i", type=int, default=3, help="Maximum worker/arbiter iterations (default: 3)")
    parser.add_argument("--investigate", action="store_true", help="Investigation only (no arbiter loop)")
    parser.add_argument("--json", action="store_true", dest="json_output", help="Output results as JSON")
    parser.add_argument("--verbose", "-v", action="store_true", help="Enable verbose logging")
    parser.add_argument("--commit", action="store_true", help="Auto-commit if accepted")
    ...
    task = Task(description=task_desc, repo_path=repo_path, files=args.files or [], commands=args.commands or [], max_iterations=args.max_iter)
...
def _print_result(result, as_json: bool) -> None:
    if as_json:
        print(json.dumps({
            ...
            "arbiter_decisions": [
                {"decision": d.decision.value, "confidence": d.confidence, "reason": d.reason,
                 "findings": [{"severity": f.severity, "description": f.description} for f in d.findings]}
...
def _print_investigation(result, as_json: bool) -> None:
    if as_json:
        print(json.dumps({"plan": result.plan, "findings": result.findings, "changes": len(result.changes), "model_used": result.model_used}, indent=2))
        return
    print(f"\nPlan: {result.plan}")
    if result.findings:
        print("\nFindings:")
